...
vtkApp.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CvtkAppApp 응용 프로그램
    클래스를 선언합니다.

vtkApp.cpp
    응용 프로그램 클래스 CvtkAppApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
vtkAppDoc.h, vtkAppDoc.cpp - 문서
    이 파일에는 CvtkAppDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CvtkAppDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.

vtkAppView.h, vtkAppView.cpp - 문서 뷰
    이 파일에는 CvtkAppView 클래스가 들어 있습니다.
    CvtkAppView 개체는 CvtkAppDoc 개체를 보는 데 사용됩니다.

res\vtkAppDoc.ico
    CvtkAppDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
